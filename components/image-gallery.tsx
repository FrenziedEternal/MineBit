...
export function ImageGallery({ images, productName, className }: ImageGalleryProps) {
  ...
  // เพิ่มฟังก์ชันตรวจสอบและแปลง YouTube URL
    ...
    <>
      <div className={cn("space-y-4", className)}>
        ...
        <div className="relative group">
          ...
          >
            {isYouTubeUrl(images[selectedImageIndex]) ? (
              <iframe
                src={getYouTubeEmbedUrl(images[selectedImageIndex])}
                className="w-full h-full"
                frameBorder="0"
                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                allowFullScreen
                title={`${productName} - วิดีโอที่ ${selectedImageIndex + 1}`}
              />
      ...
      {isModalOpen && (
        ...
        >
          ...
          >
            ...
            {isYouTubeUrl(images[modalImageIndex]) ? (
              <div className="relative w-full max-w-4xl aspect-video">
                <iframe
                  src={getYouTubeEmbedUrl(images[modalImageIndex])}
                  className="w-full h-full rounded-lg"
                  frameBorder="0"
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                  allowFullScreen
                  title={`${productName} - วิดีโอที่ ${modalImageIndex + 1}`}
                />
              </div>
